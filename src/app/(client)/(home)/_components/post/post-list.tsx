      ...
      {posts.map(post => (
        <Card key={post.id} className="space-y-4 break-all p-5">
          <div className="flex items-center gap-x-2">
            ...
            <Link className="inline-block font-title font-normal hover:text-sky-500 focus-visible:text-sky-500" href={`/posts/${post.id}`}>
              {post.title}
            </Link>
          </div>
          ...
          {post.description && <span className="line-clamp-3 text-sm text-secondary-foreground/70">{post.description}</span>}
        </Card>
      ))}
