...
export default function Page() {
  const { data: friendlinks, mutate: setFriendlinks } = useSWR('127b5c9e-fe2e-5d0a-90d1-89229101ee85', () => {
    return CustomRequest('GET api/dashboard/users/friend-links', {})
  })
  ...
  return (
    <section className="grid grid-cols-[repeat(auto-fill,minmax(300px,1fr))] gap-5">
          ...
          >
            <ModalForm
              ...
              onSubmit={async body => {
                const data = await Toast(
                  CustomRequest('PUT api/dashboard/users/friend-links/[id]', { body, params: { id: String(friendlink.id) } }),
                  {
                    success: '更新成功'
                  }
                )
            ...
            <Tooltip placement="left" title="刷新">
              <IconButton
                color="warning"
                onClick={async () => {
                  const data = await Toast(CustomRequest('PATCH api/dashboard/users/friend-links/[id]', { params: { id: String(friendlink.id) } }), {
                    success: '获取封面成功'
                  })
                  setFriendlinks(
                    produce(state => {
            ...
            <ModalDelete
              ...
              onSubmit={async () => {
                await CustomRequest('DELETE api/dashboard/users/friend-links/[id]', { params: { id: String(friendlink.id) } })
                setFriendlinks(
                  produce(state => {
      ...
      <ModalForm
        ...
        onSubmit={async body => {
          const data = await Toast(CustomRequest('POST api/dashboard/users/friend-links', { body }), {
            success: '保存成功'
          })
          setFriendlinks(
            produce(state => {
